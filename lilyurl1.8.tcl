...
# YouTube link ignore. Set to 'true' to not display YouTube links (still writes to weblog)
  ...
  if {([channel get $chan url2irc]) && ([expr [unixtime] - $url2irc(delay)] > $url2irc(last)) && (![matchattr $user $url2irc(ignore)])} {
    ...
    foreach word [split $text] {
      ...
      if {[string length $word] >= $url2irc(length) && [regexp {^http(s|)://} $word] && ![regexp {://([^/:]*:([^/]*@|\d+(/|$))|.*/\.)} $word]} {
        ...
        # add database entry before channel output formatting
        ...
        if {[regexp -nocase {(youtube|youtu.be)} $word] && $url2irc(ytignore) eq true} {
          # skip to web page output
          # putlog "URL2IRC: ytignore is 'true', skipping channel output for $url"
        } else {
          if {$url2irc(color) eq true} {
            ...
            regsub -all "Facebook" $urtitle "\00302Facebook\003" urtitle
            regsub -all "Imgur" $urtitle "\00309,01Imgur\003" urtitle
            regsub -all "Twitter" $urtitle "\00311,01Twitter\003" urtitle
          }
          # channel output
          set runtime ""
          ...
          if {[string length $newurl]} {
            puthelp "PRIVMSG $chan :\002$urtitle\002 ( $newurl ), linked by $nick $runtime"
          } else {
            puthelp "PRIVMSG $chan :\002$urtitle\002, linked by $nick $runtime"
          }
        }
...
global url2irc
  ...
  if {[info exists url] && [string length $url]} {
    ...
    # Some sites incorrectly require JS to render any title
    ...
    # if we havent returned a title yet
    if {$content_length > $url2irc(maxsize) && [string match -nocase "text/*" $content_type]} {
      putlog "URL2IRC: $url exceeded maxsize ($content_length > $url2irc(maxsize)"
    }
    if {[info exists data]} {putlog "URL2IRC: $url title data not matched"}
    return "$content_type"
...
putlog "URL 2 IRC v1.8 script loaded.."
